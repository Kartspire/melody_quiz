...
- полноценный browser-only аудиоредактор с простым и расширенным режимами, встроенным пошаговым обучением, tooltips, многодорожечным таймлайном, trim/split, ripple editing, multi-select, copy/paste, snapping, маркерами, crossfade, waveform, громкостью, fade, скоростью, mute/solo, undo/redo и WAV-рендером;
...
В аудиоредакторе новый исходник вставляется в позицию курсора. В режиме **Ripple** материал справа автоматически раздвигается при вставке и стягивается при удалении; вставка внутрь существующего клипа сначала недеструктивно разрезает его. Фрагменты магнитятся к курсору, маркерам и границам других клипов. `Ctrl/Cmd` или `Shift` позволяют выбирать несколько фрагментов, `Ctrl/Cmd+C` / `Ctrl/Cmd+V` — копировать и вставлять их с сохранением взаимного расположения. Горячие клавиши определяются по физическим клавишам (`KeyboardEvent.code`) и не зависят от текущей раскладки клавиатуры. `Ctrl/Cmd+колесо` масштабирует таймлайн относительно указателя. Базовые горячие клавиши: `Space` — play/pause, `S` — split активного фрагмента, `Delete` — удалить выделение, `Ctrl/Cmd+Z` и `Ctrl/Cmd+Shift+Z`/`Ctrl/Cmd+Y` — undo/redo.

По умолчанию аудиоредактор открывается в **простом режиме**: сложные дорожечные настройки скрыты, Ripple включён автоматически, а интерфейс показывает короткий порядок работы. Кнопка «? Как пользоваться» открывает пятишаговый встроенный гайд; при первом посещении он показывается автоматически. **Расширенный режим** возвращает Mute/Solo, несколько дорожек, маркеры, copy/paste, crossfade, fade handles, gain и скорость. Crossfade применяется сразу ко всем выбранным фрагментам: по умолчанию каждому назначается Fade In/Fade Out по 1 секунде независимо от дорожки и взаимного расположения; на коротких клипах длительность автоматически ограничивается половиной клипа. Выбранный режим и факт прохождения гайда сохраняются только как локальные UI-настройки и не меняют формат `AudioProject`.

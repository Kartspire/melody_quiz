...
- полноценный browser-only аудиоредактор: многодорожечный таймлайн, trim/split, waveform, громкость, fade, скорость, mute/solo, undo/redo и WAV-рендер;
...
В аудиоредакторе новый исходник вставляется в позицию курсора. Базовые горячие клавиши: `Space` — play/pause, `S` — split выбранного фрагмента по курсору, `Delete` — удалить выбранный фрагмент, `Ctrl/Cmd+Z` и `Ctrl/Cmd+Shift+Z`/`Ctrl/Cmd+Y` — undo/redo.

...
export const interRoundRulesChanged = createEvent<{ interRoundId: string; rules: string }>();
...
const gameEditorCommandRequested = merge([
  ...
  interRoundRulesChanged.map(({ interRoundId, rules }): GameEditorCommand => ({ type: 'changeInterRoundRules', interRoundId, rules })),

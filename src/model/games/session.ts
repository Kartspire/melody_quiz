...
export const teamNextAnswerBlockToggled = createEvent<string>();
export const teamUnlocked = createEvent<string>();
...
const gameSessionCommandRequested = merge([
  ...
  teamNextAnswerBlockToggled.map((teamId): GameSessionCommand => ({ type: 'toggleTeamNextAnswerBlock', teamId })),
  teamUnlocked.map((teamId): GameSessionCommand => ({ type: 'unlockTeam', teamId })),
